    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-10 animate-in fade-in duration-500">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={metric.label} className="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                  ...
                  >
                    <td className="px-6 py-5">
                      <div className="flex items-center space-x-4">
                        <div className={`p-2.5 rounded-xl ${vps.isConnected ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-400'}`}>
                          <Server size={18} />
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {profiles.map((vps) => (
              <div 
                key={vps.id}
                onClick={() => navigate(`/vps/${vps.id}`)}
                className="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm hover:shadow-md transition-all cursor-pointer group"
              >
                <div className="flex items-start justify-between mb-6">
                  <div className={`p-4 rounded-2xl ${vps.isConnected ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400'}`}>
                    <Server size={24} />
                  </div>
                  <button 
